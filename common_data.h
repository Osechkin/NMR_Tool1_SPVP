...
#define ALLDATA_BUFF_LEN	4096			// размер буферов данных (накопительного data_sum)
...
#define DATA_HEAP_COUNT		(2048)			// количество массивов в куче для долгосрочного хранения данных
#define UPP_DATA_COUNT		(2048)			// количество массивов для временного хранения данных АЦП (до их последующей обработки)
...
#define TELEMETRIC_DATA_LEN2 2				// количество байт измеренной температуры (2 байта) - для SPVP
#define TELEMETRIC_UART_BUF_LEN2 24			// количество байт измеренных температур (до 8 датчиков 2 байта) и напряжений (4 датчика напряжения по 2 байта) - для SPVP
...
#define NUM_CLOCKERS		5				// количество объектов-счетчиков



typedef enum {								// уникальные Id типов каротажных и лабораторных приборов ЯМР
	NMKT = 3,								// ЯМКТ
	KMRK = 4,								// КМРК
	NMR_KERN = 5,							// ЯМР-Керн
	SPVP = 6								// SPVP
...
typedef enum {								// тип измерений температуры
	TEMP_MODE_UNKNOWN = 0,					// тип измерений температуры не определен
	TEMP_MODE_NOT_SPVP = 1,					// измерение температур приборами НМКТ, КМРК и ЯМР-Керн
	TEMP_MODE_SPVP = 2						// измерение температур приборами SPVP
} TermometryMode;

typedef enum {								// флаги, свидетельствующие о готовности данных термометров
	TEMP_NOT_DEFINED = 0,					// термометры в исходном состоянии
	TEMP_STARTED = 1,						// измерения температур запущены (получен ответ о запуске измерений)
	TEMP_COUNTED = 2,						// получены данные о количестве термометров
	TEMP_READY = 3,							// данные термометров готовы (только для SPVP)
	TEMP_RESET = 4							// запущен поиск OneWire устройств (термометров)
} TermometryState;


typedef enum {								// флаги, свидетельствующие о готовности данных внутренних вольтметров
	VOLT_NOT_DEFINED = 0,					// вольтметры в исходном состоянии
	VOLT_STARTED = 1,						// измерения напряжений запущены (получен ответ о запуске измерений)
	VOLT_READY = 2							// данные напряжений готовы (только для SPVP)
} VoltageState;
...
// DataSample -------------------------------------------------------------------------------
// Контейнер для хранения данных с АЦП
typedef struct DataSample
{
	uint8_t		*data_ptr;							// указатель на источник данных (обычно данные АЦП - только для ЯМР)
	int32_t		data_len;							// длина данных (только для ЯМР)
	int32_t		echo_number;						// номер эхо (по счетчику hard_echo_counter - только для ЯМР)
	uint8_t		tool_id;							// id устройства
	uint8_t		proc_id;							// номер пакета обработки данных
	uint8_t		channel_id;							// номер канала данных
	int32_t		tag;								// дополнительное поле для отладочных целей или прочих целей пользователя
} DataSample;


// Data Heap --------------------------------------------------------------------------------
// Контейнер для хранения предобработанных данных
typedef struct DataHeap
{
	float 		*data_ptr;							// указатель на массив данных
	int32_t		data_len;							// длина данных (обычно длина спектра Фурье, хранящегося в data_ptr)
	int32_t		echo_number;						// номер массива данных (обычно номер эхо)
	uint8_t		channel_id;							// номер канала данных
	int32_t		tag;								// дополнительное поле для отладочных и прочих целей
} DataHeap;
