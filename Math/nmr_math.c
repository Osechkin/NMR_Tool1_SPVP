...
/*
//Мнимая часть принятого сигнала заполняется нулями
void signal_noise_UppDataPreprocessing3(uint8_t *upp_data, float **data_bank, float **hx_bank, int *hx_len, Processing_Params *proc_params, Data_Cmd *instr)
{
	...
	float *temp_data_ptr = data_bank[8];
	memset(temp_data_ptr, 0x00, NMR_DATA_LEN*sizeof(float));
	...
	if (win_aver == 0)
	{
		...
		{
			//dst[i] += temp_data[i]*invN;
			dst[i] = winN*dst[i]*inv_winN + temp_data_ptr[i]*inv_winN;
		}
	}
	else
	{
...
void signal_noise_UppDataPreprocessing3(uint8_t *upp_data, float **data_bank, float **hx_bank, int *hx_len, Processing_Params *proc_params, Data_Cmd *instr)
{
	...
	proc_params->points_count = src_len;
